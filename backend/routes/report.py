...
def generate_report():
    ...
    try:
        # אם לא נשלח file_id — משתמשים ב-latest
        if not file_id:
        ...
        # שליפה מהמבנה המקונן
        txn_col = (db.collection('users').document(user_id)
                   .collection('files').document(file_id)
                   .collection('transactions'))

        transactions = [doc.to_dict() for doc in txn_col.stream()]

        # שם הקובץ לכותרת
        file_doc = (db.collection('users').document(user_id)
                    .collection('files').document(file_id).get())
        file_name = file_doc.to_dict().get('file_name', '') if file_doc.exists else ''
        report_title = f"Cardify - Report: {file_name.rsplit('.', 1)[0]}" if file_name else "Cardify - Transaction Report"
        ...
        story += [
            ...
            Paragraph(f"Account: {username}  |  Date: {datetime.now().strftime('%d/%m/%Y %H:%M')}", small_style),
        ...
        irregular_indices = [i+1 for i, t in enumerate(sorted_txns) if t.get('status') == 'IRREGULAR']
        row_colors = []
